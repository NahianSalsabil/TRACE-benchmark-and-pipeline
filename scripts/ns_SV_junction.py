...
def find_junction_options(road_id, routes):
    for entry in routes:
        if entry['connected_road_id'] == road_id:
            return entry['junction_id'], entry['junction_road_id']
...
def spawn_vehicle(xodrPath, pointsPath, routePath, reportPath, simulation_path):
    ...
    try:
        ...
        print(f"An error occurred while spawning the vehicle: {e}")
