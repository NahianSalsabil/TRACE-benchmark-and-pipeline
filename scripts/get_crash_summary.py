...
def extract_crash_data(xml_file_path):
    try:
        ...
        # Define the main namespace URI
        NS_URI = 'http://schemas.datacontract.org/2004/07/NHTSA.FARS.Data'
...
def main():
    input_file_dir = 'Crash_Reports/detailed_reports23_veh-2'
    output_file_dir = 'Crash_Reports/Summary_Manual'
    for filename in os.listdir(input_file_dir):
        if filename.endswith(".xml"):
            ...
            output_file_path = os.path.join(output_file_dir, f"summary_{filename[5:].replace('.xml', '.txt')}")
    ...
    print(f"Data successfully written to '{output_file_dir}'")
